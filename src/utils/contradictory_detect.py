# NOTE: Added package: sentence-transformers, torch, einops, hf_xet
...
class ContradictionDetector:
    ...
    def get_embeddings(self, sentences, task_type="search"):
        """Get embeddings with task-specific prefixes for better performance"""
        prefixed_sentences = [f"Are they contradictory? {sent}" for sent in sentences]

        embeddings = self.model.encode(prefixed_sentences, normalize_embeddings=True, prompt_name=task_type)

        return embeddings

    # @deprecated(reason="Use detect_contradiction_contrast instead")
    ...
    def detect_contradiction_contrast(self, sentence1, sentence2, task_type="passage") -> dict:
        """Advanced method: Use contrastive embeddings
        Create positive and negative versions to better detect contradictions
        """
        # Original sentences
        original_emb = self.get_embeddings([sentence1, sentence2], task_type)
        ...
        negative_emb = self.get_embeddings([neg_sentence1, neg_sentence2], task_type)

        # Calculate similarities
        original_sim = cosine_similarity([original_emb[0]], [original_emb[1]])[0][0]
        cross_sim1 = cosine_similarity([original_emb[0]], [negative_emb[0]])[0][0]
        cross_sim2 = cosine_similarity([original_emb[1]], [negative_emb[1]])[0][0]
        ...
        is_contradiction = contradiction_score > original_sim and abs(contradiction_score - original_sim) > 0.01
    ...
    def batch_detect_contradictions(self, sentence_pairs) -> list[dict]:
...
def main():
    ...
    detector_nomic = ContradictionDetector("nomic-ai/nomic-embed-text-v2-moe")
    ...
    for pair in test_cases:
        ...
        print(f"Contrast Method - Contradiction: {result_contrast['is_contradiction']}, correct: {pair[2]}")
        print(f"Contradiction Score: {result_contrast['contradiction_score']:.3f}")
        print(f"Original Similarity: {result_contrast['original_similarity']:.3f}")
        print(f"Confidence: {result_contrast['confidence']:.3f}")
